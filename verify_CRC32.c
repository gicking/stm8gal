...
uint8_t upload_crc32_code(HANDLE ptrPort, uint8_t physInterface, uint8_t uartMode)
{
  ...
  if (!(imageBuf = malloc(LENIMAGEBUF * sizeof(*imageBuf))))
	...
	hexfile_convertIHex((char*) bin_verify_CRC32_ihx, bin_verify_CRC32_ihx_len, imageBuf, MUTE);

	// get image size
	hexfile_getImageSize(imageBuf, 0, LENIMAGEBUF, &addrStart, &addrStop, &numData);
...
{
	uint8_t		status;
	uint32_t	crc32_uC, crc32_PC, numTx;
	...
	while (addr <= addrStop) {
		...
		{
			char Tx = NACK;
			send_port(ptrPort, 0, 1, &Tx, &numTx);
		}
